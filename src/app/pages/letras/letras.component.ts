...
export class LetrasComponent implements OnInit {
  ...
  filterPdfs() {
    ...
    this.filteredPdfs = this.pdfs
      ...
      .filter((pdf) =>
        removeAccents(pdf.title.toLowerCase()).includes(
          removeAccents(this.searchQuery.toLowerCase())
        )
      );
  }
  ...
  private zoomToPoint(clientX: number, clientY: number, zoomFactor: number) {
    if (this.pdfViewer) {
      const containerRect =
        this.pdfViewer.nativeElement.getBoundingClientRect();
      ...
      if (oldZoom !== this.zoom) {
        const scrollElement = this.pdfViewer.nativeElement.querySelector(
          '.ng2-pdf-viewer-container'
        );
        if (scrollElement) {
          const newScrollLeft =
            (scrollElement.scrollLeft + offsetX * containerRect.width) *
              (this.zoom / oldZoom) -
            offsetX * containerRect.width;
          const newScrollTop =
            (scrollElement.scrollTop + offsetY * containerRect.height) *
              (this.zoom / oldZoom) -
